...
import com.yiij.web.interfaces.IWidgetFactory;
 ...
 * @version $Id$
 */
public class WidgetFactory extends WebApplicationComponent implements
		IWidgetFactory {

	private ComponentConfig _widgets = new ComponentConfig();
	
	public WidgetFactory(IContext context) {
		super(context);
	}
	...
	/**
	 * @return widget initial property values. Each array key-value pair
	 ...
	 */
	public ComponentConfig getWidgets()
	{
		return _widgets;
	}
	 ...
	 * @see #getWidgets()
	 */
	public void setWidgets(ComponentConfig value)
	{
		_widgets = value;
	}
	 ...
	 * @param properties the initial property values (ComponentConfig) or class name of the widget.
	 * @return the newly created widget whose properties have been initialized with the given values.
	 * @throws InstantiationException 
	 */
	@Override
	public Widget createWidget(BaseController owner, Object properties) throws InstantiationException {
		
		ComponentConfig config = new ComponentConfig();
		String className = (properties instanceof ComponentConfig?((ComponentConfig)properties).className : (String)properties);
		if (_widgets != null && _widgets.containsKey(className))
			config.merge(_widgets.get(className));
		config.merge(properties);
		
		return (Widget)Component.newInstance(context(), config, owner);
	}
